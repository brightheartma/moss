...
调查开始时，Moss 仍锁定 Vitest 3 和 TypeScript 5.9。下面记录的是**迁移前基线及其因果分析**，用于说明为什么升级 TypeScript 本身不能解除“必须有工具 lowering 装饰器”的约束：
...
- 迁移前的 `Vitest 3 -> Vite 7 -> esbuild(target: es2022)` 会 lowering 装饰器；本仓库的 [Vitest 配置](../../packages/core/vitest.config.ts) 继续显式提供这个 target。
...
- 迁移前的 TypeScript 5.9 pin 是另一条独立约束：`tsup --dts` 与 TypeScript 6/7 的兼容，而不是装饰器支持。TypeScript 7 又没有稳定 programmatic API，所以它比原文只提到的“等待 tsup 支持 TS 6”更需要单独规划。
...
| **调查开始时的 Moss 基线** | **TypeScript 5.9.3** | 当时的 `tsup --dts` 在 TS6 上仍有 `baseUrl` 兼容问题，TS7 又没有它所需的旧 API |
...
因此，对“有没有支持 TS7 的 tsup 替代品”的最短答案是：**有，tsdown 已有 TS7/tsgo 声明路径；但由于 Moss 使用 standard decorators，它还不是 drop-in replacement。Moss 不需要再找一个 bundler 来替代 tsup，当前直接使用 TypeScript 6 CLI；TypeScript 7 留待其 API 与生态进一步成熟后评估。**
...
第二条关键结论是：TypeScript 版本与 Vitest/Oxc 的 runtime transform 是两条独立约束。本次使用 TypeScript 6 CLI 移除了 `tsup --dts` 依赖；以后评估 TypeScript 7 时仍应单独验证其 API 与生态兼容性。

## 贡献文档表述

> Moss 使用 standard decorators（TypeScript 5+ 的 `(value, context)` / 2022-03 语义族；TC39 当前为 Stage 2.7），并保持 `experimentalDecorators` 关闭。Node 目前不能解析未转换的 decorator syntax，因此 build、test 和直接执行 TypeScript 的每条路径都必须使用已验证的 decorator-lowering transformer。包构建使用 TypeScript 6 CLI 与 `target: ES2022`；测试使用 Vitest 4 + Vite 7/esbuild，并显式设置相同 target。升级到 Vite 8 前，必须先验证 Babel/SWC workaround，或等待 Oxc 原生 lowering。
>
> TypeScript 固定为 `~6.0.0`，直接发布 `tsc` 生成的多文件 ESM 与声明树，不使用 tsup，也不执行声明 bundling。TypeScript 7.0 尚无稳定 programmatic API，升级时必须重新验证编译、声明、消费者类型检查和生态兼容性。
